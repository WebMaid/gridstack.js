...
export class GridStack {
  ...
  public constructor(el: GridHTMLElement, opts: GridStackOptions = {}) {
    ...
    let defaults: GridStackOptions = {
      ...
      column:
        Utils.toNumber(el.getAttribute("gs-column")) || gridDefaults.column,
      minRow: rowAttr
        ? rowAttr
      ...
      removableOptions: {
        accept: opts.itemClass
          ? "." + opts.itemClass
          : gridDefaults.removableOptions.accept,
      },
    };
    ...
    let engineClass =
      this.opts.engineClass || GridStack.engineClass || GridStackEngine;
    ...
    if (this.opts.auto) {
      ...
      this.getGridItems().forEach((el) => {
        // get dom elements (not nodes yet)
        ...
        elements.push({
          ...
          i:
            ...
            (Number.isNaN(y) ? 1000 : y) * column,
        });
      });
      elements
        .sort((a, b) => b.i - a.i)
        .forEach((e) => this._prepareElement(e.el)); // revert sort so lowest item wins
  ...
  public addWidget(
    ...
    function isGridStackWidget(w: GridStackWidget): w is GridStackWidget {
    ...
    if (typeof els === "string") {
      ...
      arguments.length === 0 ||
      (arguments.length === 1 && isGridStackWidget(els))
    ) {
      node = options = els;
      if (node?.el) {
        ...
        let doc = document.implementation.createHTMLDocument(""); // IE needs a param
        doc.body.innerHTML = `<div class="grid-stack-item ${
          this.opts.itemClass || ""
        }"><div class="grid-stack-item-content">${content}</div></div>`;
        el = doc.body.children[0] as HTMLElement;
      }
    } else {
  ...
  public makeSubGrid(
    ...
    ops = Utils.cloneDeep(
      ops ||
        (node.subGrid as GridStackOptions) || {
          ...this.opts.subGrid,
          children: undefined,
        }
    );
    ...
    let content = node.el.querySelector(
  ...
  public save(
    ...
    list.forEach((n) => {
      if (saveContent && n.el && !n.subGrid) {
        ...
        if (n.subGrid) {
          n.subGrid = (n.subGrid as GridStack).save(
            saveContent,
            true
          ) as GridStackOptions;
        }
  ...
  public load(
    ...
    addRemove: boolean | AddRemoveFcn = this.opts.addRemoveCB || true
  ): GridStack {
    let items = GridStack.Utils.sort(
      [...layout],
      -1,
      this._prevColumn || this.getColumn()
    ...
    if (typeof addRemove === "function")
      this.opts.addRemoveCB = addRemove as AddRemoveFcn;
    ...
    items.forEach((w) => {
      let item =
        w.id || w.id === 0
          ? this.engine.nodes.find((n) => n.id === w.id)
          : undefined;
      if (item) {
        this.update(item.el, w);
        if (w.subGrid && (w.subGrid as GridStackOptions).children) {
  ...
  public getCellFromPixel(
    ...
    let rowHeight =
      box.height / parseInt(this.el.getAttribute("gs-current-row"));
  ...
  public setStatic(
    ...
    this.engine.nodes.forEach((n) => {
      ...
      if (n.subGrid && recurse)
        (n.subGrid as GridStack).setStatic(val, updateClass, recurse);
    });
    if (updateClass) {
      this._setStaticClass();
    }
  ...
  protected _updateStyles(forceUpdate = false, maxH?: number): GridStack {
    ...
    if (!this._styles) {
      ...
      let styleLocation = this.opts.styleInHead
        ? undefined
        : (this.el.parentNode as HTMLElement);
      this._styles = Utils.createStylesheet(
      ...
      Utils.addCSSRule(
        this._styles,
        prefix,
        `min-height: ${cellHeight}${cellHeightUnit}`
      );
      // content margins
    ...
    if (maxH > this._styles._max) {
      let getHeight = (rows: number): string =>
        cellHeight * rows + cellHeightUnit;
  ...
  public onParentResize(): GridStack {
    ...
    if (this._isAutoCellHeight) {
      if (!changedColumn && this.opts.cellHeightThrottle) {
        if (!this._cellHeightThrottle) {
          this._cellHeightThrottle = Utils.throttle(
            () => this.cellHeight(),
    ...
    this.engine.nodes.forEach((n) => {
      if (n.subGrid) {
        (n.subGrid as GridStack).onParentResize();
      }
    });
  ...
  public enableMove(doEnable: boolean, recurse = true): GridStack {
    ...
    this.engine.nodes.forEach((n) => {
      ...
      if (n.subGrid && recurse)
        (n.subGrid as GridStack).enableMove(doEnable, recurse);
    });
    return this;
  }
  ...
  public enableResize(doEnable: boolean, recurse = true): GridStack {
    ...
    this.engine.nodes.forEach((n) => {
      ...
      if (n.subGrid && recurse)
        (n.subGrid as GridStack).enableResize(doEnable, recurse);
    });
    return this;
  }
        ...
        (event, el: GridItemHTMLElement, helper: GridItemHTMLElement) => {
          ...
          if (helper !== el) {
            ...
            if (this.opts.disableRemoveNodeOnDrop) {
              ...
              // Fixes issue with react virtual dom
  ...
  protected _dragOrResize(
    ...
    if (event.type === "drag") {
      ...
      let left =
        ui.position.left +
        (ui.position.left > node._lastUiPosition.left ? -mRight : mLeft);
      let top =
        ui.position.top +
        (ui.position.top > node._lastUiPosition.top ? -mBottom : mTop);
      ...
      if (
        node._lastTried &&
        node._lastTried.w === p.w &&
        node._lastTried.h === p.h
      )
    ...
    let rect: GridStackPosition = {
      // screen pix of the dragged box
    ...
    if (
      this.engine.moveNodeCheck(node, {
        ...p,
        cellWidth,
        cellHeight,
        rect,
      ...
      if (resizing && node.subGrid) {
        (node.subGrid as GridStack).onParentResize();
      } // @ts-ignore
      this._extraDragRow = 0; // @ts-ignore
      this._updateContainerHeight();
